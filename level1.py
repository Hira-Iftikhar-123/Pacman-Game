...
game_over_font = pygame.font.SysFont("timesnewroman",60)
...
for i in range (1,5):
    ...
    GAME_IMAGES.append(pygame.transform.scale(pygame.image.load(image_path),(30,28)))
...
# creating class ghost because all ghosts have different movements so, we can make 4 instances and move every each ghost different from other
class Ghost:
...
def welcome_screen():
    ...
    while True:
        ...
        text = font.render("Welcome to PACMAN!", True, WHITE)
...
def the_game_board(lvl):
    ...
    for i in range (len(lvl)):
        for j in range (len(lvl[i])):
            if lvl[i][j] == 1:
                pygame.draw.circle(SCREEN,'yellow',(j * num2 + (0.5 * num2),i * num1 + (0.5 * num1)),3)
            if lvl[i][j] == 2 and not flicker:
                pygame.draw.circle(SCREEN,'yellow',(j * num2 + (0.5 * num2),i * num1 + (0.5 * num1)),9)
            if lvl[i][j] == 3:
...
def draw_footer():
    ...
    for i in range(pacman_lives):
        SCREEN.blit(pygame.transform.scale(GAME_IMAGES[0], (30, 30)), (520 + i * 40, 665))
    if game_over:
    ...
    if game_won:
        ...
        pygame.draw.rect(SCREEN,'white',[40,180,580,220],0,10)
        game_won_text = font.render('Congratulations! Press Tab to go to new level',True,'green')
        SCREEN.blit(game_won_text,(100,300))     
    if game_start:
        pygame.draw.rect(SCREEN,'white',[20,160,620,260],0,10)
        pygame.draw.rect(SCREEN,'white',[40,180,580,220],0,10)
        game_start_text = font.render('Welcome to PACMAN. Press 1 to start!',True,'black')
        SCREEN.blit(game_start_text,(100,300))     
...
while running:
    ...
    player_circle = pygame.draw.circle(SCREEN, 'black', (center_x, center_y - 2), 16, 2)
    ...
    remaining_dots = sum(row.count(1) for row in level)
    
    if remaining_dots == 0:
        game_won = True
        print("All dots eaten! Switching to Level 2...")
        import level2 
        level2.start_level2() 
    ...
    if game_won:
        game_won_sound = pygame.mixer.Sound('gallery/sounds/pacman_intermission.wav')
        game_won_sound.play()
        level = copy.deepcopy(boards)
